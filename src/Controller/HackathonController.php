...
class HackathonController extends AbstractController
{
    ...
    public function hackathonHtmlId(EntityManagerInterface $entityManager, int $id): Response
    {
        ...
        if (!$hackathon) {
            throw $this->createNotFoundException('No hackathon found for id '.$id);
        }
        return new Response(
    ...
    #[Route('/api/hackathon/{id}', name: 'hackathon_id')]
    public function hackathonJsonId(EntityManagerInterface $entityManager, int $id): JsonResponse
    {
        $hackathon = $entityManager->getRepository(Hackathon::class)->find($id);
        if (!$hackathon) {
            throw $this->createNotFoundException('No hackathon found for id '.$id);
